...
def showVersion():
    ...
    try:
        if latestVersion == "":
            tprint(
                "Current knoxnl version " + __version__ + " (unable to check if latest)"
            )
        elif __version__ == latestVersion:
            tprint(
                "Current knoxnl version "
                + __version__
                + " ("
...
def showBanner():
    ...
    try:
        ...
        if currentDate.month == 12 and currentDate.day in (24, 25):
            ...
            tprint(
                colored(" *** 🥳 HAPPY NEW YEAR!! 🥳 ***", "yellow", attrs=["blink"])
            )
...
def showBlocked():
    ...
    try:
        ...
        domainsBlockedLimit = [
            domain
            for domain, count in blockedDomains.items()
            if count > args.skip_blocked - 1
        ]
        if domainsBlockedLimit:
            ...
            tprint(
                colored(
                    "The following domains seem to be blocking KNOXSS and might be worth excluding for now:",
                    "yellow",
                ),
                colored(domainList, "white"),
            )
    except Exception:
        pass
        ...
        try:
            if len(inputValues) > 0 and not args.no_todo:
                try:
                    tprint(
                        colored(
                            "\n>>> Just trying to save outstanding input to .todo file before ending...",
                            "yellow",
                        )
                    )
                    with open(todoFileName, "w") as file:
                        for inp in inputValues:
                            file.write(inp + "\n")
                    tprint(
                        colored("All unchecked URLs have been written to", "cyan"),
                        colored(todoFileName + "\n", "white"),
                    )
                except Exception as e:
                    tprint(
                        colored("Error saving file ", "cyan"),
                        colored(todoFileName + "\n", "white"),
                        colored(":" + str(e), "red"),
                    )
        except Exception:
    ...
    try:
        ...
        if args.burp_piper:
            print(
                ...
                "Request passed from Burp Piper Extension",
            )
        else:
            if urlPassed:
                print(
                    colored("-i: " + args.input, "magenta"),
                    "The URL to check with KNOXSS API.",
                )
            else:
                print(
                    colored("Discord Webhook Complete:", "magenta"),
                    DISCORD_WEBHOOK_COMPLETE,
                )
                print(
                    colored("-i: " + args.input + " (FILE)", "magenta"),
                    "All URLs will be passed to KNOXSS API.",
                )

        if fileIsOpen:
            print(
                colored("-o: " + args.output, "magenta"),
                "The output file where successful XSS payloads will be saved.",
            )
            print(
            ...
            print(
                colored("-oa: " + str(args.output_all), "magenta"),
                "Whether the output all results to the output file, not just successful one's.",
            )

        if debugFileIsOpen:
            print(
                colored("-do: " + args.debug_output, "magenta"),
                "The output file where all debug information will be saved.",
            )

        if not urlPassed:
            print(
                colored("-p: " + str(args.processes), "magenta"),
                "The number of parallel requests made, i.e. number or processes/threads.",
            )

        print(
            colored("-X: " + args.http_method, "magenta"),
            "The HTTP method checked by KNOXSS API.",
        )

        if args.http_method in ("POST", "BOTH"):
            if args.post_data != "":
                print(
                    colored("-pd: " + args.post_data, "magenta"),
                    "Data passed with a POST request.",
                )
            else:
                if urlPassed:
                    try:
                        ...
                        postData = ""
                    print(
                        colored("-pd: " + postData, "magenta"),
                        "Data passed with a POST request.",
                    )
                else:
                    print(
                        colored("-pd: {the URL query string}", "magenta"),
                        "Data passed with a POST request.",
                    )
        ...
        if args.headers != "":
            print(
                ...
                "HTTP Headers passed with requests.",
            )

        print(
            colored("-t: " + str(args.timeout), "magenta"),
            "The number of seconds to wait for KNOXSS API to respond.",
        )
        print(
            colored("-st: " + str(args.stall_timeout), "magenta"),
            "The number of seconds to wait for the KNOXSS API scan to take between steps before aborting.",
        )

        if args.retries == 0:
            print(
                colored("-r: " + str(args.retries), "magenta"),
                "If having issues connecting to the KNOXSS API, the program will not sleep and not try to retry any URLs. ",
            )
        else:
            print(
                colored("-r: " + str(args.retries), "magenta"),
                "The number of times to retry when having issues connecting to the KNOXSS API. The number of retries will also be reset every 24 hours when running for a file.",
            )
        if args.retries > 0:
            print(
                colored("-ri: " + str(args.retry_interval), "magenta"),
                "How many seconds to wait before retrying when having issues connecting to the KNOXSS API.",
            )
            print(
                colored("-rb: " + str(args.retry_backoff), "magenta"),
                "The backoff factor used when retrying when having issues connecting to the KNOXSS API.",
            )

        if args.skip_blocked > 0:
            print(
                colored("-sb: " + str(args.skip_blocked), "magenta"),
                "The number of 403 Forbidden responses from a target (for a given HTTP method + scheme + (sub)domain) before skipping.",
            )

        if args.force_new:
            print(
                colored("-fn: True", "magenta"),
                "Forces KNOXSS to do a new scan instead of getting cached results.",
            )

        if args.runtime_log:
            print(
                colored("-rl: True", "magenta"),
                "Provides a live runtime log of the KNOXSS scan.",
            )

        if args.no_todo and not urlPassed:
            print(
                colored("-nt: True", "magenta"),
                "If the input file is not completed because of issues with API, connection, etc. the .todo file will not be created.",
            )

        if timeAPIReset is not None:
            print(
                colored("KNOXSS API Limit Reset Time:", "magenta"),
                str(timeAPIReset.strftime("%Y-%m-%d %H:%M")),
            )
            if args.pause_until_reset:
                print(
                    colored("-pur: True", "magenta"),
                    "If the API limit is reached, the program will pause and then continue again when it has been reset.",
                )
        else:
            if args.pause_until_reset:
                print(
                    colored("-pur: True", "magenta"),
                    "NOT POSSIBLE: Unfortunately the API reset time is currently unknown, so the program cannot be paused and continue when the API limit is reached.",
                )
        print()
...
def needApiKey():
    ...
    try:
        tprint(
            colored(
                "Haven't got an API key? Why not head over to https://knoxss.pro and subscribe?\nDon't forget to generate and SAVE your API key before using it here! 🤘\n",
                "green",
            )
        )
    except Exception:
...
def getConfig():
    ...
    try:
        ...
        try:
            ...
            tprint(
                colored('Unable to read "API_URL" from config.yml; defaults set', "red")
            )
        ...
        try:
            ...
            if args.api_key != "":
                ...
                tprint(
                    colored(
                        'NOTE: Overriding "API_KEY" from config.yml with passed API Key',
                        "cyan",
                    ),
                    API_KEY + "\n",
                )
                ...
                if API_KEY is None or API_KEY == "YOUR_API_KEY":
                    tprint(
                        colored(
                            'ERROR: You need to add your "API_KEY" to config.yml or pass it with the -A option.',
                            "red",
                        )
                    )
                    needApiKey()
            ...
            tprint(
                colored(
                    'Unable to read "API_KEY" from config.yml; We need an API key! - '
                    + str(e),
                    "red",
                )
            )
        ...
        if args.discord_webhook != "":
            ...
            try:
                DISCORD_WEBHOOK = config.get("DISCORD_WEBHOOK").replace(
                    "YOUR_WEBHOOK", ""
                )
            except Exception:
                DISCORD_WEBHOOK = ""
        ...
        if args.discord_webhook_complete != "":
            ...
            try:
                DISCORD_WEBHOOK_COMPLETE = config.get(
                    "DISCORD_WEBHOOK_COMPLETE"
                ).replace("YOUR_WEBHOOK", "")
            except Exception:
                DISCORD_WEBHOOK_COMPLETE = ""
        ...
        try:
            if args.api_key == "":
                tprint(
                    colored(
                        "Unable to read config.yml and API Key not passed with -A; Unable to use KNOXSS API! - "
                        + str(e),
                        "red",
                    )
                )
                ...
                tprint(
                    colored(
                        "Unable to read config.yml; using default API URL and passed API Key",
                        "cyan",
                    )
                )
            DISCORD_WEBHOOK = args.discord_webhook
...
def tprint(*arguments, sep=" ", end="\n"):
    ...
    try:
        ...
        if (verbose() or args.runtime_log) and (args.processes > 1 and not urlPassed):
            ...
            if not re.match(r"^\[\d{2}:\d{2}:\d{2}\]: ", message) or (
                verbose() or args.runtime_log
            ):
...
def knoxssApi(targetUrl, headers, method, knoxssResponse):
    ...
    try:
        ...
        while tryAgain:
            ...
            try:
                while True:
                    ...
                    try:
                        ...
                        if (
                            args.runtime_log or verbose() or debugFileIsOpen
                        ) and line.startswith("["):
                            tprint(line)
            ...
            try:
                knoxssResponse.Code = (
                    str(shared["status_code"]) if shared["status_code"] else "Unknown"
                )
                if connectionError:
                    ...
                    knoxssResponse.Error = (
                        f"The scan exceeded the timeout of {args.timeout} seconds."
                    )
                    knoxssResponse.PoC = "none"
                    knoxssResponse.Calls = "Unknown"
                    ...
                    if jsonPart == "Invalid or expired API key.":
                        ...
                        knoxssResponse.Calls = str(
                            jsonResponse.get("API Call", "Unknown")
                        )
                        ...
                        if knoxssResponse.PoC != "none":
                            if (
                                ...
                                if not (
                                    timeAPIReset is not None and args.pause_until_reset
                                ):
                                    needToStop = True
                            else:
...
def checkForAlteredParams(url):
    ...
    try:
        if "=FUZZ" in url or "=Gxss" in url:
            tprint(
                colored(
                    "WARNING: It appears the URL may have been manually changed by yourself (or another tool) first. KNOXSS might not work as expected without the default values of parameters (some parameters might be value sensitive). Just pass original URLs to knoxnl.",
                    "yellow",
                )
            )
        ...
        if regexCheckCompiled.search(url):
            tprint(
                colored(
                    "WARNING: It appears the URL may already include some XSS payload. If that's correct, KNOXSS won't work as expected since it's not meant to receive XSS payloads, but to provide them.",
                    "yellow",
                )
            )
    except Exception as e:
    ...
    try:
        ...
        if args.burp_piper:
            try:
                ...
                args.input = (
                    "https://"
                    + secondLine.split(" ")[1].strip()
                    + firstLine.split(" ")[1].strip()
                )
                ...
                while header.strip() != "":
                    if (
                        header.lower().find("cookie") >= 0
                        or header.lower().find("api") >= 0
                        or header.lower().find("auth") >= 0
                    ):
                ...
                print(
                    colored(
                        "ERROR: Burp Piper Extension input expected: " + str(e), "red"
                    )
                )
                exit()
        else:
            ...
            if not args.input:
                print(
                    colored(
                        "ERROR: The -i / --input argument must be passed (unless calling from Burp Piper extension with -bp / --burp-piper). The input can be a single URL or a file or URLs.",
                        "red",
                    )
                )
                exit()
            ...
            if "://" not in inputArg:
                print(
                    colored(
                        'WARNING: Input "'
                        + inputArg
                        + '" should include a scheme. Using https by default...',
                        "yellow",
                    )
                )
...
def discordNotify(target, poc, vulnType):
    ...
    try:
        ...
        if attempt == max_attempts and (
            ...
            tprint(
                colored(
                    "ERROR: Failed to send notification to Discord after "
                    + str(max_attempts)
                    + " attempts.",
                    "red",
                )
    ...
    try:
        ...
        while attempt < max_attempts:
            ...
            if result.status_code == 429:
                ...
                tprint(
                    colored(
                        "ERROR: Failed to send Complete notification to Discord - "
                        + result.text,
                        "yellow",
                    )
                )
        ...
        if attempt == max_attempts and (
            ...
            tprint(
                colored(
                    "ERROR: Failed to send Complete notification to Discord after "
                    + str(max_attempts)
                    + " attempts.",
                    "red",
                )
...
def getAPILimitReset():
    ...
    try:
        ...
        if timeAPIReset is not None and (datetime.now() - timeAPIReset) > timedelta(
            hours=24
        ):
            timeAPIReset = None
...
def processOutput(target, method, knoxssResponse):
    ...
    try:
        ...
        if knoxssResponse.Calls == "Unknown" and all(
            s not in knoxssResponseError.lower()
            for s in ["content type", "can't test it (forbidden)"]
        ):
            ...
            if "got another 403" in knoxssResponseError.lower():
                knoxssResponseError = (
                    'Target returned a "403 Forbidden". There could be WAF in place.'
                )
                ...
                if args.skip_blocked > 0:
                    try:
                        ...
                        domain = (
                            ...
                            + method
                            + ") "
                            + str(parsedTarget.scheme + "://" + parsedTarget.netloc)
                        )
                        pauseEvent.set()
            ...
            if (
                knoxssResponse.PoC != "none"
                and knoxssResponse.XSS != "true"
                and knoxssResponse.Redir != "true"
            ):
            ...
            if knoxssResponse.PoC == "none":
                if not args.success_only:
                    ...
                    if urlPassed:
                        ...
                        tprint(
                            colored(xssText, "yellow"),
                            colored(errorText, "red"),
                            colored("[" + latestApiCalls + "]", "white"),
                        )
                    if args.output_all and fileIsOpen:
                        outFile.write(xssText + "\n")
                ...
                # This shouldn't happen, but just in case, output an error if verbose was chosen
                    ...
                    if verbose() or debugFileIsOpen:
                        tprint(
                            colored(
                                "ERROR: There was a PoC, but didn't seem to be an XSS or OR. Check the response for details:",
                                "red",
                            )
                        )
...
def processUrl(target):
    ...
    try:
        ...
        if not stopProgram and not needToStop:
            ...
            if (
                rateLimitExceeded
                and timeAPIReset is not None
                and args.pause_until_reset
            ):
                # Set the event to pause all processes
                pauseEvent.set()
                tprint(
                    colored(
                        f'WAITING UNTIL {str(timeAPIReset.strftime("%Y-%m-%d %H:%M"))} WHEN THEN API LIMIT HAS BEEN RESET...',
                        "yellow",
                    )
                )
                ...
                if retryAttempt < args.retries:
                    ...
                    if retryAttempt == 0:
                        ...
                        delay = args.retry_interval * (
                            retryAttempt * args.retry_backoff
                        )
                    if retryAttempt == args.retries:
                        tprint(
                            colored(
                                "WARNING: There are issues with KNOXSS API. Sleeping for "
                                + str(delay)
                                + " seconds before trying again. Last retry.",
                                "yellow",
                            )
                        )
                    else:
            ...
            if args.http_method in ("GET", "BOTH"):
                ...
                if (
                    args.skip_blocked > 0
                    and blockedDomains[domain] > args.skip_blocked - 1
                ):
                    tprint(
                        colored(
                            "[ SKIP ] - "
                            + domain
                            + " has already been flagged as blocked, so skipping "
                            + target,
                            "yellow",
                            attrs=["dark"],
                        )
...
def processes_type(x):
    ...
    if x < 1 or x > 5:
        raise argparse.ArgumentTypeError(
            "The number of processes must be between 1 and 5"
        )
    return x
...
def updateProgram():
    try:
        ...
        print(
            colored(
                f"knoxnl successfully updated {__version__} -> {latestVersion} (latest) 🤘",
                "green",
            )
        )
        ...
        print(
            colored(
                f"Unable to update knoxnl to version {latestVersion}: {str(e)}", "red"
            )
        )
...
def main():
    ...
    parser.add_argument(
        "-t",
        "--timeout",
        help="How many seconds to wait for the KNOXSS API to respond before giving up (default: "
        + str(DEFAULT_TIMEOUT)
        + " seconds). If set to 0, then timeout will be used.",
    ...
    parser.add_argument(
        "-st",
        "--stall-timeout",
        help="How many seconds to wait for the KNOXSS API scan to take between steps before aborting (default: "
    ...
    parser.add_argument(
        "-r",
        "--retries",
        help="The number of times to retry when having issues connecting to the KNOXSS API (default: "
        + str(DEFAULT_RETRIES)
        + "). If set to 0 then then it will not sleep or try to retry any URLs.",
    ...
    parser.add_argument(
        "-ri",
        "--retry-interval",
        help="How many seconds to wait before retrying when having issues connecting to the KNOXSS API (default: "
        + str(DEFAULT_RETRY_INTERVAL)
        + " seconds)",
    ...
    parser.add_argument(
        "-rb",
        "--retry-backoff",
        help="The backoff factor used when retrying when having issues connecting to the KNOXSS API (default: "
        + str(DEFAULT_RETRY_BACKOFF_FACTOR)
        + ")",
    ...
    try:
        resp = requests.get(
            "https://raw.githubusercontent.com/xnl-h4ck3r/knoxnl/main/knoxnl/__init__.py",
            timeout=3,
        )
        latestVersion = resp.text.split("=")[1].replace('"', "").strip()
    except Exception:
        pass
    ...
    if args.update:
        try:
            if latestVersion == "":
                print(
                    colored(
                        "Unable to check the latest version. Check your internet connection.",
                        "red",
                    )
                )
            elif __version__ != latestVersion:
                ...
                print(
                    colored(
                        "You are already running the latest version of knoxnl.Thanks for using!",
                        "green",
                    )
                )
            print()
    ...
    if args.output != "":
        try:
            ...
            print(
                colored(
                    "WARNING: Output won't be saved to file - " + str(e) + "\n", "red"
                )
            )
    ...
    if args.debug_output != "":
        try:
            ...
            print(
                colored(
                    "WARNING: Debug output won't be saved to file - " + str(e) + "\n",
                    "red",
                )
            )
    ...
    try:
        ...
        if needToStop and not urlPassed and not args.burp_piper and not args.no_todo:
            try:
                ...
                print(
                    colored(
                        "Had to stop due to errors. All unchecked URLs have been written to",
                        "cyan",
                    ),
                    colored(todoFileName + "\n", "white"),
                )
        ...
        try:
            ...
            if successCountXSS > 0 and successCountOR > 0:
                ...
                if successCountXSS > 0:
                    message = (
                        "🤘 " + str(successCountXSS) + " successful XSS found! 🤘\n"
                    )
                    print(colored(message, "green"))
                elif successCountOR > 0:
                    message = (
                        "🤘 No successful XSS, but "
                        + str(successCountOR)
                        + " successful "
                        + openRedirectTerm
                        + " found! 🤘\n"
        ...
        if debugFileIsOpen:
            try:
                ...
                print(
                    colored(
                        "ERROR: Unable to close debug output file: " + str(e), "red"
                    )
                )
